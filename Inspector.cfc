<cfscript>
	component{
		function init(any object, errorOnUndefined="false"){
			...
			variables.componentAnnotations = {};
			variables.flattenedProperties = flattenProperties();
		...
		private struct function flattenProperties(Struct metadata="#getMetadata(variables.object)#",struct props="#{}#",numeric counter="1"){
			var prop = 1;
			var arguments.name = "";
			var properties = [];
			...
			for(annotation in metadata){
				if(NOT arrayFindNoCase(variables.builtinComponentPropertyList,annotation)
					AND NOT StructKeyExists(variables.componentAnnotations,annotation)
				){
					variables.componentAnnotations[annotation] = metadata[annotation];
				}
			}
			//climb the tree
			if(StructKeyExists(metadata,"extends")){
				props = flattenProperties(metadata.extends,props,counter);
			}
			return props;
		}
